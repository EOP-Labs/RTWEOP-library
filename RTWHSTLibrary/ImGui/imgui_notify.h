...
public:
	...
	NOTIFY_INLINE auto set_type(const ImGuiToastType& new_type) -> void { IM_ASSERT(type < ImGuiToastType_COUNT); this->type = new_type; };
...
namespace ImGui
{
	...
	NOTIFY_INLINE void RenderNotifications()
	{
		...
		for (auto i = 0; i < (int)notifications.size(); i++)
		{
